    ...
    <div className={`header header_bg-color_${bgColor}`}>
      ...
      > 
        ...
        <div className="header__wrapper">
          ...
          <Link className={`header__sign-text header__sign-text_color_${textColor}`} to={`${pathname === '/' ? '/movies' : '/profile'}`}>{text}</Link>
          {pathname === '/' ? 
            <button className="header__btn-signin" type="button">Войти</button>: <button className="header__btn-account" type="button" />}
        </div>
      </div>
    </div>
  );
